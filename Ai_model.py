...
def clean_text(text):
    ...
    corrected_text = spell.correction(text) or ""
    if re.match(r'^[A-Z\s]+$', corrected_text) and len(corrected_text) > 1:
    ...
    for i, (bbox, text) in enumerate(text_output):
        ...
        if clean_name:
            ...
            rooms[room_key] = {
                ...
                "sizes": [],
                "bbox": bbox,
                "text_elements": [(bbox, text)],  # Add text_elements key with initial bounding box and text
            ...
            for j in range(i + 1, len(text_output)):
                ...
                if next_y > room_y and (next_y - room_y) < 50 and any(char.isdigit() for char in next_text):
                    rooms[room_key]["sizes"].append(next_text)
                    rooms[room_key]["text_elements"].append((next_bbox, next_text))  # Add size bounding box and text
                    break

    for room_key, details in rooms.items():
        if not details["sizes"]:
            # Add coordinates as a fallback when no size is mentioned
...
def rename_room_size(command, rooms, original_image):
    ...
    size_match = re.search(r"rename\s+(\w+)\s+size\s+(\d+x\d+)\s+to\s+(\d+x\d+)", command, re.IGNORECASE)

    if size_match:
        room_name, old_size, new_size = size_match.groups()
        ...
        if room_name in rooms:
            ...
            room_info = next((room for room in rooms[room_name] if old_size in room["sizes"]), None)
            if room_info:
                # Iterate over text output to identify the bounding box associated specifically with the size
                size_bbox = None
                for bbox, text in room_info["text_elements"]:
                    if text == old_size:  # Look for bounding box specifically of old size text
                        size_bbox = bbox
                        break
                ...
                if size_bbox:
                    ...
                    # Update the room size in the room's details
                    room_info["sizes"] = [new_size if s == old_size else s for s in room_info["sizes"]]
        ...
        response = "Could not understand the size rename command. Use the format: 'rename <room name> size <old size> to <new size>'."
...
def rename_room(command, rooms, original_image):
    ...
    if coord_match:
        ...
        # Flag to check if renaming was successful
        renamed = False

        # Check if the room name exists in the dictionary
        if room_name in rooms:
            # Iterate through each room detail in the list
            for room_info in rooms[room_name]:
                ...
                if room_info["coordinates"] == (x, y):
                    ...
                    # Update the displayed text on the image
                    bbox = room_info['bbox']
                    ...
                    # Erase the old room name text
                    cv2.rectangle(updated_image, (x, y), (x + w, y + h), bg_color, -1)

                    # Add the new room name text at the same location
                    font_scale = room_info.get("font_size", 0.9)
                    cv2.putText(updated_image, new_name, (x, y + h - 5), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 0), 2)
                    break
    ...
    # Case 2: Renaming room by size
        ...
        size_match = re.search(r"rename\s+(\w+)\s+(\d+x\d+)\s+to\s+(\w+)", command, re.IGNORECASE)
        if size_match:
            old_name, old_size, new_name = size_match.groups()
            old_name, old_size, new_name = old_name.upper(), old_size.upper(), new_name.upper()

            if old_name in rooms:
                room_info = next((room for room in rooms[old_name] if old_size in room["sizes"]), None)
                if room_info:
                    bbox = room_info['bbox']
                    x, y, w, h = cv2.boundingRect(np.array([list(point) for point in bbox], dtype=np.int32))
                    margin = 5
                    ...
                    # Update the room size in the room's details
                    room_info["sizes"] = [new_name if s == old_size else s for s in room_info["sizes"]]
                    response = f"Updated size of '{old_name}' from '{old_size}' to '{new_name}' in the blueprint."
                else:
                    response = f"Could not locate bounding box for size '{old_size}' in room '{old_name}'."
            else:
                response = f"Room '{old_name}' with size '{old_size}' not found. Ensure the room and size are correctly detected in the blueprint."
        else:
            response = "Could not understand the rename command. Use the format: 'rename <room name> size <old size> to <new size>' or 'rename <room name> coordinates: (<x>, <y>) to <new name>'."
